...
import { REQUEST_PULLREQUESTS, RECEIVE_PULLREQUESTS, REQUEST_PULLREQUEST, RECEIVE_PULLREQUEST, SUBMIT_PULLREQUEST } from '../reducers/pullRequests'
...
export const fetchPullRequests = () => async (dispatch, getState) => {
  ...
  const pullRequests = await Promise.all(_.range(numPRs).map(getPullRequestByIndex))
...
const submitPullRequest = pullRequest => ({
 ...
 export const createPullRequest = ({ taskId, url}) => async (dispatch, getState) => {
